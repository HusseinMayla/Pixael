...
export const StateManager: React.FC = () => {
  ...
  return (
    <div className="flex items-center gap-1.5 overflow-x-auto py-1 scrollbar-none">
      <div className="flex items-center gap-1 text-slate-400 text-xs font-medium mr-1.5 shrink-0">
      ...
      {asset.states.map((st) => {
        ...
        return (
          <div key={st.id} className="relative shrink-0 flex items-center">
            {isEditing ? (
              <div className="flex items-center gap-1 px-2 py-1 bg-studio-950 border border-accent-500 rounded-lg">
                <input
                  type="text"
                  value={editName}
                  ...
                  autoFocus
                  className="w-20 px-1 py-0.5 text-xs bg-transparent text-white focus:outline-none"
                />
                <button
              ...
              <div
                className={`group flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all cursor-pointer ${
                  isActive
            ...
            {/* Dropdown Menu */}
            {isMenuOpen && (
              <div className="absolute top-full mt-1.5 left-0 z-40 bg-studio-900 border border-studio-700/80 rounded-lg shadow-xl py-1 min-w-[130px] animate-in fade-in zoom-in-95 duration-150">
                <button
      ...
      {isAddingState ? (
        <form onSubmit={handleCreateState} className="flex items-center gap-1 px-2 py-1 bg-studio-950 border border-accent-500 rounded-lg shrink-0">
          <input
            type="text"
            value={newStateName}
        ...
        <Tooltip content="Add Animation State">
          <button
            ...
            className="p-1.5 rounded-lg bg-studio-900/60 hover:bg-studio-800 border border-dashed border-studio-700 hover:border-accent-500 text-slate-400 hover:text-white transition-colors shrink-0 flex items-center gap-1 text-xs"
          >
            <Plus className="w-3.5 h-3.5" />
            <span className="text-[11px] pr-1">New State</span>
          </button>
        </Tooltip>
      )}
    </div>
